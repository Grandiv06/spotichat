...
import { emitSocket, getSocket, onSocketEvent } from "@/lib/socket";
...
type TypingKind = "text" | "voice" | "video";
...
export const chatService = {
  ...
  // Load a reasonable number of messages for performance, especially on mobile.
  // If you need full history, we can later add \"load older messages\" with real pagination.
  getMessages: async (
    chatId: string,
    page = 1,
    limit = 50,
  ...
  sendMessage: async (
    messageData: Omit<Message, "id" | "createdAt" | "status">,
  ): Promise<Message> => {
    const wsPayload = {
  ...
  sendMediaMessage: async (
    ...
    formData.append(
      "file",
      file,
      `${type}_${Date.now()}.${type === "voice" ? "webm" : "webm"}`,
    );
  ...
  onMessageStatus: (
    callback: (data: { id: string; chatId: string; status: string }) => void,
  ) => {
    return onSocketEvent("message:status", callback);
  },

  onTypingStart: (
    callback: (data: { chatId: string; userId: string; kind?: TypingKind }) => void,
  ) => {
  ...
  startTyping: (chatId: string, kind: TypingKind = "text") => {
    emitSocket("typing:start", { chatId, kind });
  },
